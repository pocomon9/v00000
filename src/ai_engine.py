...
class PocoAI:
    ...
    def _publicity_stunt_hook(self, topic: str, limit: int = 240) -> str:
        topic_l = (topic or "general").lower()
        if "crypto" in topic_l:
            hooks = [
                "Style court is open: is this actual taste, crowd psychology, or a trend wearing sunglasses?",
                "Tiny timeline trial: remove the hype, keep the liquidity. What still looks bullish?",
                "Plot twist test: which breaks first here, narrative, positioning, or patience?",
            ]
        elif "war" in topic_l or "geo" in topic_l or "politic" in topic_l:
            hooks = [
                "Geopolitics court is open: who benefits if everyone keeps watching the loudest headline?",
                "One-map challenge: ignore the speech, follow the chokepoint. What changes?",
                "Plot twist test: the public story is loud, but the incentive map is louder.",
            ]
        elif "meme" in topic_l or "entertainment" in topic_l:
            hooks = [
                "Timeline court: is this actually funny, or did the internet just need a new villain for the afternoon?",
                "Caption trial: the joke works because everyone knows exactly who it is about.",
                "Tiny chaos test: if this keeps spreading, what is the one line people will repeat?",
            ]
        else:
            hooks = [
                "Timeline court is open: what is the real reason this caught fire?",
                "One-question test: is this signal, theatre, or both wearing the same coat?",
                "Plot twist check: the loud part is obvious. The useful part is what people are avoiding.",
            ]
        return self._clean_generated_post(hooks[random.randrange(len(hooks))])[:limit]

    def _maybe_publicity_stunt(self, text: str, topic: str, *, limit: int = 240) -> str:
        chance = max(0, min(100, _env_int("poco_PUBLICITY_STUNT_CHANCE_PERCENT", 12)))
        if chance <= 0 or random.randint(0, 99) >= chance:
            return self._clean_generated_post(text)[:limit]
        return self._publicity_stunt_hook(topic, limit=limit)
    ...
    def _candidate_passes_comment_quality(self, candidate: dict) -> bool:
        ...
        replies = metrics["replies"]
        views = metrics["views"]
        min_likes = _env_int("poco_MIN_COMMENT_SOURCE_LIKES", max(1200, _env_int("poco_MIN_SOURCE_LIKES", 1800) // 2))
        min_replies = _env_int("poco_MIN_COMMENT_SOURCE_REPLIES", 45)
        min_views = _env_int("poco_MIN_COMMENT_SOURCE_VIEWS", 120000)
        small_likes = _env_int("poco_MIN_DISCUSSION_SOURCE_LIKES", 250)
        small_replies = _env_int("poco_MIN_DISCUSSION_SOURCE_REPLIES", 8)
        small_combined = _env_int("poco_MIN_DISCUSSION_SOURCE_COMBINED", 420)
        loyal_replies = _env_int("poco_MIN_LOYAL_DISCUSSION_REPLIES", 4)
        loyal_likes = _env_int("poco_MIN_LOYAL_DISCUSSION_LIKES", 40)
        loyal_ratio = float(os.environ.get("poco_MIN_DISCUSSION_REPLY_RATIO", "0.08") or "0.08")
        if not str(candidate.get("source_url", "")).strip():
            return False
        if views and views >= min_views:
            return True
        if replies >= min_replies and (likes >= min_likes or reposts >= max(80, min_likes // 20)):
            return True
        combined = likes + (reposts * 4) + (replies * 5) + int(views / 150)
        if combined >= _env_int("poco_MIN_COMMENT_SOURCE_COMBINED", 1800):
            return True
        discussion_ratio = replies / max(likes + reposts, 1)
        if replies >= loyal_replies and likes >= loyal_likes and discussion_ratio >= loyal_ratio:
            return True
        return replies >= small_replies and (likes >= small_likes or combined >= small_combined)
    ...
    def _fallback_engagement_comment(self, candidate: dict) -> str:
        ...
        if any(token in topic for token in ("art", "literature", "poetry", "cinema", "music", "culture", "vintage")):
            base = f"The quiet part is the timing. Everyone is arguing the headline while the map moves underneath. {self._question_suffix(topic)}"
        elif any(token in topic for token in ("politics", "political", "election", "policy", "government")):
            base = f"The headline is loud, but the incentives are steering the car. {self._question_suffix(topic)}"
        elif any(token in topic for token in ("fashion", "beauty", "style", "vogue", "aesthetic", "runway", "couture")):
            base = f"The candle is the distraction; positioning is the plot. {self._question_suffix(topic)}"
        elif any(token in topic for token in ("entertainment", "celebrity", "movie", "music", "gaming", "streaming")):
            base = f"This is attention economics with better lighting. {self._question_suffix(topic)}"
        elif any(token in topic for token in ("meme", "humor", "funny", "viral")):
            base = f"This meme is doing free political science in one frame. {self._question_suffix(topic)}"
        else:
            lead = hook.rstrip(" .!?,:;") if hook else "The underrated signal is hiding in plain sight"
            base = f"{lead}. {self._question_suffix(topic)}"
        return self._public_safe_text(base, limit=220)
    ...
    def _generate_engagement_comment(self, candidate: dict) -> str:
        ...
        reply = self._clean_generated_post(self.llm.ask(prompt, timeout=30, role="creator"))
        if not reply:
        ...
        reply = self._maybe_publicity_stunt(reply, str(candidate.get("topic", "")).strip() or "general", limit=220)
    ...
    def _rephrase_candidate(self, candidate: dict) -> str:
        ...
        response = self._maybe_publicity_stunt(response, str(candidate.get("topic", "")).strip() or "general", limit=240)
        if self._question_post_enabled():
    ...
    def check_mentions(self) -> int:
        ...
        for mention in mentions[:5]:
            ...
            reply = self._public_safe_text(self.llm.ask(prompt, timeout=30, role="creator"), limit=220)
            if not reply:
